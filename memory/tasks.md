...
- **Updated**: 2026-03-06 15:31 UTC
- **Notes**: Пользователь просит идеальный режим: один общий copy/paste запуск (single-shot), отдельные шаги только для критичных мест (например, ввод пароля). Дальше выдавать единый скрипт-команду с пометками, где может потребоваться отдельный ввод.
