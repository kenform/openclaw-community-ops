...
- **Updated**: 2026-03-13 06:25 UTC
- **Notes**: Пользователь подтвердил автономный режим B, риск 0.25% от депозита на сделку (по стопу), и осторожный human-like темп без параллельного спама сделок.
- **Result**: Стартую безопасный поэтапный прогон: проверка интерфейса/доков -> калибровка размера позиции -> последовательные сделки с логом.
