...
- **Updated**: 2026-03-06 12:07 UTC
- **Notes**: Приоритет на сегодня подтверждён пользователем: доделать задачи по Eragon-сайту в первую очередь (tailnet отложить). Пользователь запустил `npm run dev`; пойман Runtime Error от браузерного расширения Talisman (`chrome-extension://...`), не от кода Next.js. Следующий шаг: дать обход (отключить extension на localhost/использовать чистый профиль) и подтвердить запуск страницы.
