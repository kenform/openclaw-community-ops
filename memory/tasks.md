...
- **Updated**: 2026-03-13 06:39 UTC
- **Notes**: Пользователь прислал полный деплой-чеклист для VPS (ssh, python venv, config.env, systemd, анализ). Для старта нужны подтверждения и секреты/артефакты: источник файлов проекта, Telegram bot token/chat, Upscale demo credentials, и явное разрешение на elevated команды (sudo/systemctl).
- **Result**: Готова начать сразу после получения минимальных входных данных и подтверждения режима (в параллель с текущим demo-тестом или после него).
